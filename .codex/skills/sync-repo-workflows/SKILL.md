...
1. 먼저 `docs(haru2)/reference/repo-workflow-parity.md`를 읽는다
2. 아래 whitelist 경로만 비교 대상으로 삼는다
   - `AGENTS.md`
   - 루트: `docs(haru2)/README.md`
   - 루트: `docs(haru2)/reference/document-writing-rules.md`
   - 루트: `docs(haru2)/troubleshooting/README.md`
   - 루트: `docs(haru2)/runbooks/README.md`
   - 인프라 계열: `docs/README.md`
   - 인프라 계열: `docs/reference/document-writing-rules.md`
   - 인프라 계열: `docs/troubleshooting/README.md`
   - 인프라 계열: `docs/runbooks/README.md`
...
- 루트 `deep-quest`는 현재 `docs(haru2)/`를 canonical docs 경로로 사용하고, `infra` 계열은 `docs/`를 사용한다는 점을 전제로 비교한다
- 경로가 다르더라도 의도와 문서 역할이 같으면 parity 후보로 본다
- `docs(haru2)/status/**`, `docs(haru2)/tasks/**`, `docs/status/**`, `docs/tasks/**`, 개별 `runbooks/*.md`, 개별 `troubleshooting/*.md`, `docs/infra_state/**`는 비교만 하더라도 기본적으로 동기화 대상에서 제외한다
