...
1. 먼저 `docs/reference/repo-workflow-parity.md`를 읽는다
...
6. 먼저 차이 목록을 정리하고, 그 다음 공통 축만 선택적으로 동기화한다
7. 공통 skill이 `bkit` 초안을 전제로 하면, 루트를 제외한 인프라 두 저장소에서는 대응하는 `scripts/commands/`와 `scripts/bkit/*`까지 같이 갱신한다
8. 외부 저장소를 수정할 때는 저장소별 git 상태를 먼저 확인하고, 변경 범위를 분리한다
9. 동기화 후에는 저장소별로 무엇이 같아졌고 무엇을 의도적으로 남겼는지 요약한다
