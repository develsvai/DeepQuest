...
`현재 저장소`의 상태 문서 흐름에 `bkit`을 붙일 때 필요한 입력 수집과 프롬프트 번들 생성을 이 디렉터리에서 관리한다.
...
- 자동화가 작업 축을 바꾸지 않도록, 변환 대상은 `new-status`, `plan-task`, `update-task`, `update-troubleshooting`, `create-runbook`으로 제한한다
...
- `scripts/commands/create-runbook`

이 명령들은 각각 같은 이름의 워크플로를 바로 실행한다.
현재 skill 흐름도 이 명령들을 먼저 시도한 뒤, 결과를 검토해 실제 문서 반영 여부를 판단하는 방식으로 연결돼 있다.

주요 스크립트:

- `prepare-workflow-context.sh`
  - `new-status`, `plan-task`, `update-task`, `update-troubleshooting`, `create-runbook`용 `prompt.md`, `context.md`, `sources.txt` 번들을 만든다
  ...
  - 예시: `infra/scripts/bkit/prepare-workflow-context.sh create-runbook --print-prompt`
- `run-workflow.sh`
...
- `bash infra/scripts/commands/create-runbook --print-output`
