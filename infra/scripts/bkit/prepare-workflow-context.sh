...
case "$WORKFLOW" in
  new-status|plan-task|update-task|update-troubleshooting|create-runbook)
    ;;
  *)
...
RUNBOOKS_README="$RUNBOOKS_DIR/README.md"

PROMPT_FILE="$OUT_DIR/prompt.md"
CONTEXT_FILE="$OUT_DIR/context.md"
SOURCES_FILE="$OUT_DIR/sources.txt"

LATEST_STATUS="$(latest_file_optional "$STATUS_DIR" 'status-*.md')"
LATEST_TASK="$(latest_file_optional "$TASKS_DIR" 'task-*.md')"
...
case "$WORKFLOW" in
  ...
  update-troubleshooting)
    ...
    ;;
  create-runbook)
    SKILL_FILE="$SKILLS_DIR/create-runbook/SKILL.md"
    TARGET_PATH="$RUNBOOKS_DIR"
    ;;
esac
...
case "$WORKFLOW" in
  new-status)
    ...
    ;;
  plan-task)
    [[ -n "$LATEST_STATUS" ]] || die "plan-task requires at least one status file in $STATUS_DIR"
    ;;
  update-task)
    [[ -n "$LATEST_TASK" ]] || die "update-task requires at least one task file in $TASKS_DIR"
    ;;
  update-troubleshooting)
    ;;
esac

{
  ...
  case "$WORKFLOW" in
    ...
    update-troubleshooting)
      ...
      ;;
    create-runbook)
      printf '작업 기록에서 반복 가능한 운영 절차를 추려 `runbooks/`용 재현 가능한 문서 초안을 만든다.\n'
      ;;
  esac
  printf '\n## Guardrails\n\n'
  ...
  case "$WORKFLOW" in
    ...
    update-troubleshooting)
      ...
      ;;
    create-runbook)
      printf -- '- 상단 메타 정보와 `목적 -> 사전 확인 사항 -> 실행 순서 -> 검증 방법 -> 주의 사항` 흐름을 가진 runbook 초안\n'
      ;;
  esac
  printf '\n## Source Files\n\n'
  printf -- '- `%s`\n' "${README_DOC#$ROOT_DIR/}"
...
append_file_block "$CONTEXT_FILE" "$RUNBOOKS_README"
...
if [[ -n "$LATEST_STATUS" ]]; then
  SOURCE_ITEMS+=("${LATEST_STATUS#$ROOT_DIR/}")
fi
if [[ -n "$LATEST_TASK" ]]; then
  SOURCE_ITEMS+=("${LATEST_TASK#$ROOT_DIR/}")
fi
