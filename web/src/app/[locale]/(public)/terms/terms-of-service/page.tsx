...
export async function generateMetadata(): Promise<Metadata> {
  ...
  return {
    ...
    description: t('terms.termsOfServiceDescription'),
  }
}
 ...
 *
 * Renders the terms of service document in the current locale.
 * Falls back to Korean (default) if the translated version is not available.
 */
export default async function TermsOfServicePage() {
  const locale = await getLocale()
  const content = await getMarkdownContent('terms-of-service.md', locale)
